...
opts = optimoptions('ga','ConstraintTolerance',1e-10,'PlotFcn', @gaplotbestf);
...
vet = load("Vettore Juno.txt");
r_vero = vet(1:10:end, 1:3)';
dth = pi / 1000;
%%
clc, close all
drawPlanet(10, [0 0 0], 20, 1)
plotOrbitVet(r_vero, 2.5, 'r', '--')
plotorbit(kep_arc1, kep_arc1(6), kep_arc1(6) + dth1, dth, ksun, 'b')
plotorbit(kep_arc2, kep_arc2(6), kep_arc2(6) + dth2, dth, ksun, 'b')
plotorbit(kep_arc3, kep_arc3(6), kep_arc3(6) + dth3, dth, ksun, 'b')
legend("","Ephemerides Juno trajectory","Calculated Juno trajectory")
%%
e_cap = 0.9884;
v_inf = dV_2; 
dV_peri = v_inf*sqrt((1-e_cap)/2);
